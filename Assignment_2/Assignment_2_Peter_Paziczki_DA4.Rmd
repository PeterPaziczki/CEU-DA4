...
#data <- data[complete.cases(data[,c("profit_loss_2011", "profit_loss_2012","profit_loss_2013", "share_eq_2011", "share_eq_2012", "share_eq_2013")])]

```

#### 1.2 Calibration example with age

I am running a logit linear regression to predict firm default with only one predictor, age.
...
logit <- glm(defaulted ~ poly(age, 3), family = "binomial", data=data)
logit
...
m2 <- get(Y) ~ poly(age, 3)
m3 <- get(Y) ~ poly(age, 3)
m4 <- get(Y) ~ age + poly(age, 2) + poly(age, 3) + region + f_ind + size_cat + sales_cat_2012 + sales_cat_2013 + income_cat_2012 + income_cat_2013
...
mrf <- f_defaulted ~ age + agesq + agecu + f_region + agegroup + f_ind + size_cat + sales_cat_2012 + sales_cat_2013 + income_cat_2012 + income_cat_2013
